    ...
    if host_count == 0:
       raise NoRepositories
        ...
        for i in range(host_count):
            d = {}
            d['nr'] = i + 1
            for k in ( 'naam', 'url', 'host', 'user', 'password', 'basedir'):
                d[ k ] =  cfg.get('Repo' + str(i+1), k)

            __repoindex[ d['naam'] ] = len(__repositories)
            __repositories.append( Repository(d['nr'],d['naam'],d['url'],d['host'],d['user'],d['password'],d['basedir']) )
